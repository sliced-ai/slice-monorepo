...
cfg = {
    "experiment_name": "experiments/0_context_fast_larger_1lr",  # Add your experiment name here
    "main_model": {
    ...
    "decoding_strategy": "top_k",  # Decoding strategy: 'greedy', 'top_k'
...
class CombinedTrainer:
    def __init__(self, cfg, rw_loader, pile_loader, tokenizer):
        ...
        self.epoch_log_df = pd.DataFrame(columns=["epoch", "step", "train_loss", "word_correct", "total_predictions"])
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            while True:
                ...
                # Perform evaluation every 20 steps
                if step_number % 100 == 0:
                    word_correct, total = self.evaluate_and_save_results(rw_batch, epoch + 1, step_number)

                    # Print epoch, step, loss, and evaluation results
                    print(f"Epoch {epoch + 1}, Step {step_number}, Train Loss: {loss.item()}, Word Prediction: {word_correct}/{total}")
                ...
                log_row = pd.DataFrame([{
                    ...
                    "word_correct": word_correct if step_number % 100 == 0 else None,
                    "total_predictions": total if step_number % 100 == 0 else None
                }])
    ...
    def evaluate_and_save_results(self, rw_batch, epoch, step):
        ...
        return word_correct, total_word
        ...
        for sentence in sentences:
            ...
            with torch.no_grad():
                if self.cfg["decoding_strategy"] == "greedy":
                    ...
                    # Top-k sampling
                    output = self.model.generate(input_tokens, attention_mask=attention_mask, max_length=input_tokens.size(1) + 1, do_sample=True, top_k=self.cfg["top_k"], pad_token_id=self.tokenizer.pad_token_id)
