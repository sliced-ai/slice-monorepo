...
utterances = process_folder(FOLDER_PATH, TARGET_NAME, NUM_UTTERANCE_PAIRS)

print(f"Total utterance pairs loaded: {len(utterances)}")
print(f"Names included in the data: {TARGET_NAME}")

print("\nTwo random examples of instruction-response pairs:")
for i in range(2):
    example = random.choice(utterances)
...
dataset = UtteranceDataset(utterances, tokenizer)
