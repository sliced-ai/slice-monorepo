...
def evaluate_loss_and_accuracy(model, tokenizer, question, answer, lr):
    ...
    return avg_train_loss, loss_after, correct_count
...
def find_best_lr_for_epoch(learning_rates, steps, question, answer, model_name, epoch):
    ...
    print(f"Processing question: {question}, Epoch: {epoch}")
    ...
    while global_step < steps:
        ...
        avg_train_loss, loss_after, correct_count = evaluate_loss_and_accuracy(model, tokenizer, question, answer, lr)
        global_step += 1
        ...
        print(f"Step {global_step}/{steps}: Random LR = {lr}, Train Loss = {avg_train_loss}, Inference Loss = {loss_after}, Correct Count = {correct_count}")
        ...
        if loss_after < best_value:
            ...
            print(f"New local minimum found at step {global_step} with LR = {lr} and Inference Loss = {loss_after}")
        ...
        if loss_after > 5 * best_value or loss_after > 5:
            print(f"Skipping stepping for LR = {lr} due to high inference loss after backpropagation")
            del model
            torch.cuda.empty_cache()
            gc.collect()
            continue
        ...
        while left_idx > 0 and global_step < steps:
            ...
            if model_name != MODEL_NAME:
                model.load_state_dict(torch.load(model_name))
            avg_train_loss, loss_after, correct_count = evaluate_loss_and_accuracy(model, tokenizer, question, answer, prev_lr)
            global_step += 1
            result = {
                "Question": question,
            ...
            print(f"Step {global_step}/{steps}: Left LR = {prev_lr}, Train Loss = {avg_train_loss}, Inference Loss = {loss_after}, Correct Count = {correct_count}")
        ...
        while right_idx < len(learning_rates) - 1 and global_step < steps:
            ...
            if model_name != MODEL_NAME:
                model.load_state_dict(torch.load(model_name))
            avg_train_loss, loss_after, correct_count = evaluate_loss_and_accuracy(model, tokenizer, question, answer, next_lr)
            global_step += 1
            result = {
                "Question": question,
            ...
            print(f"Step {global_step}/{steps}: Right LR = {next_lr}, Train Loss = {avg_train_loss}, Inference Loss = {loss_after}, Correct Count = {correct_count}")
...
for question, answer in zip(qa_data["question"], qa_data["answer"]):
    ...
    while True:
        results, best_min, best_value, best_correct_count, best_model = find_best_lr_for_epoch(learning_rates, STEPS, question, answer, model_name, epoch)
        print(f"Best learning rate found for question '{question}', epoch {epoch}: {learning_rates[best_min]} with inference loss {best_value}")
        ...
        fine_results, fine_best_min, fine_best_value, fine_best_correct_count, fine_best_model = find_best_lr_for_epoch(fine_tuned_lrs, FINE_TUNING_STEPS, question, answer, model_name, epoch)
        print(f"Best fine-tuned learning rate found for question '{question}', epoch {epoch}: {fine_tuned_lrs[fine_best_min]} with inference loss {fine_best_value}")

        # Save the fine-tuned model with the lowest loss
        last_word = question.rstrip("?").split()[-1]
        fine_model_save_path = os.path.join(MODEL_SAVE_DIR, f"fine_model_best_{last_word}_lr_{fine_tuned_lrs[fine_best_min]}_epoch_{epoch}.pt")
        ...
        if fine_best_correct_count / BATCH_SIZE >= ACCURACY_THRESHOLD:
            print(f"Accuracy threshold met for question '{question}' at epoch {epoch}")
            break
