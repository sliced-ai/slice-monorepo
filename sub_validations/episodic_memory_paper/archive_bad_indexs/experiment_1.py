...
cfg = {
    ...
    "inference_batch_size": 100,  # Number of inferences done in parallel
    "max_inference_steps": 100,   # Total number of inference steps
    "num_train_epochs": 2,
    "pile_training_steps": 50,   # Number of steps to train on Pile data
    "pile_token_size": 2049,      # Size of tokens for Pile dataset
    "pile_inference_interval": 1, # Reinference every n steps on the Pile data
    "experiment_name": "single_example_inference_4_greedy",
    ...
    def train_on_pile(self, pile_loader, qa_data, qa_index, num_steps, interval, experiment_folder, model_name):
        ...
        for step, batch in enumerate(pile_loader):
            ...
            if (step + 1) % interval == 0:
                ...
                # Save forgetting results to CSV (save only one reinference step per step)
...
def run_experiment(cfg):
    ...
    for model_name in cfg['model_names']:
        ...
        qa_loader = DataLoader(qa_dataset, batch_size=cfg['training_batch_size'], shuffle=False)
