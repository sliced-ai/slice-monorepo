...
cfg = {
    ...
    "num_train_epochs": 3,
    "pile_training_steps": 10,
    "pile_inference_interval": 1,
    "experiment_name": "layer_freeze",
...
class CustomTrainer:
    def __init__(self, cfg, qa_loader, tokenizer, model_name, experiment_folder):
        ...
        # Freeze the first 3/4 of the model layers
        self.freeze_lower_layers(self.model)

    def freeze_lower_layers(self, model):
        """
        Freeze the lower 3/4 of the model's transformer layers.
        """
        num_layers = len(model.gpt_neox.layers)
        num_layers_to_freeze = int(3 * num_layers / 4)  # Freeze 3/4 of the layers
        for i, layer in enumerate(model.gpt_neox.layers):
            if i < num_layers_to_freeze:
                for param in layer.parameters():
                    param.requires_grad = False
        print(f"Froze the first {num_layers_to_freeze} layers out of {num_layers} layers")
