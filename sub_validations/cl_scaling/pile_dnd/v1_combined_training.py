...
cfg = {
    ...
    "experiment_name": "combined_training_4",
...
class CombinedTrainer:
    def __init__(self, cfg, rw_loader, pile_loader, tokenizer):
        ...
        # Print the calculated learning rate scheduler details
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i in range(len(self.rw_loader)):
                ...
                rw_inference_loss, pile_inference_loss = self.calculate_inference_loss(rw_batch, pile_batch)

                print(f"Epoch {epoch + 1}/{self.num_epochs}, Step {i + 1}/{len(self.rw_loader)}: Train Loss = {loss.item()}, Learning Rate = {self.scheduler.get_last_lr()[0]}, RW Inference Loss = {rw_inference_loss}, Pile Inference Loss = {pile_inference_loss}")
