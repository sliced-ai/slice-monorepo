...
class CustomNeuron(nn.Module):
    ...
    def forward(self, input):
        ...
        selection_logits = self.selection_layer(input_detached)  # [b, s, o*n]
        ...
        outputs_all = torch.einsum('bsi,nio->bsno', input, comp_weights_transposed)  # [b, s, n, o]

        # Add biases if present
        if self.comp_biases is not None:
            comp_biases = self.comp_biases.unsqueeze(0).unsqueeze(0)  # [1, 1, n, o]
            outputs_all = outputs_all + comp_biases  # [b, s, n, o]

        # Apply selected mask
        selected_mask_transposed = selected_mask.permute(0, 1, 3, 2)  # [b, s, n, o]
        output = torch.sum(outputs_all * selected_mask_transposed, dim=2)  # Sum over n -> [b, s, o]
