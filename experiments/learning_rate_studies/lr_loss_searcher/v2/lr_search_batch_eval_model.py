...
def ensure_dirs(cfg, experiment_name):
    """
    Ensures that the necessary directories for saving models and results exist.
    """
...
def gen_lrs(lr_range, num_pts=3, fine_tune=False, center_lr=None):
    """
    Generates a list of learning rates for the search.
    If fine_tune is True, generate a finer range around the center_lr.
    """
    if fine_tune:
...
def clean_mem(model):
    """
    Frees up memory by deleting the model and clearing the GPU cache.
    """
    del model
...
class QADataset(Dataset):
    """
    Custom Dataset class for handling QA pairs.
    """
...
def main():
    ...
    qa_data = load_data('data_single.json')
    ...
    while True:
        ...
        eval_model_weights = best_model_path_eval if eval_model_name is not None else None
        epoch += 1
    
        clean_mem(best_model)
        if eval_model_name:
            clean_mem(best_model_eval)
